...
def about(keywords: list[str]):
    ...
    >>> pick(['Cute Dog!', 'That is a cat.', 'Nice pup!'], about_dogs, )
    'Cute Dog!'
...
def autocorrect(typed_word: str, word_list: list[str], diff_function, limit: int) -> str:
    ...
    'butter'
    >>> first_diff = lambda w1, w2, limit: (1 if w1[] != w2[] else ) # Checks for matching first char
...
def final_diff(typed: str, source: str, limit: int) -> int:
    ...
    # 剪枝
    if typed == source: return 0
    if typed.lower() == source.lower(): return 0 # 考虑英语大小写的情况，故模糊处理
    t, s = len(typed), len(source)
    if limit < 0: return limit + 1
    if not t or not s: return t + s
    if abs(t - s) > limit:
        return limit + 1
    #搜索
    if typed[0] == source[0]:
        ...
        rem = 1 + final_diff(typed[1:], source, limit - 1)
        dis = round(KEY_DISTANCES.get((typed[0], source[0]), 1.0))
        sub = dis + final_diff(typed[1:], source[1:], limit - dis)
        swap = (
            1 + final_diff(typed[2:], source[2:], limit - 1)
            if (t > 1 and s > 1 
                and typed[0] == source[1] and typed[1] == source[0]) 
            else limit + 1
        )
        return  min(add, rem, sub, swap)
...
def report_progress(typed: list[str], source: list[str], user_id: int, upload) -> float:
    ...
    >>> print_progress({'id': 1, 'progress': .6})
    ID: 1 Progress: .6
    >>> typed = ['how', 'are', 'you']
...
def fastest_words(words_and_times: dict) -> list[list[str]]:
    ...
    >>> p0 = [5, 1, 3]
    >>> p1 = [4, 1, 6]
    >>> fastest_words({'words': ['Just', 'have', 'func'], 'times': [p0, p1]})
    [['have', 'func'], ['Just']]
    ...
    for word_idx in range(len(words)):
        ...
        result[best_player].append(words[word_idx])
    return result
    # END PROBLEM 10
...
def get_time(times, player_num, word_index):
    ...
    assert word_index < len(times[0]), f"word_index {word_index} outside of  to {num_words-1}"
    assert player_num < len(times), f"player_num {player_num} outside of  to {num_players-1}"
